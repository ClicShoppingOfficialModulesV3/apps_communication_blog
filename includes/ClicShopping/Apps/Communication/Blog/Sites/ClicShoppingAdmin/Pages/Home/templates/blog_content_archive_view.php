...
<?php
  ...
  if (isset($cPath)) {
    $cPath = HTML::sanitize($cPath);
  } else {
    $cPath = 0;
  }

  echo HTML::form('goto', $CLICSHOPPING_Blog->link('BlogContent'), 'post', null, ['session_id' => true]);
  echo HTML::selectMenu('cPath', BlogAdmin::getBlogCategoryTree(), $cPath, 'onchange="this.form.submit();"');
?>
              </form>
            </div>
          </span>
...
<?php
  ...
  if (!isset($_POST['search'])) {
    echo HTML::button($CLICSHOPPING_Blog->getDef('button_new_category'), null, $CLICSHOPPING_Blog->link('BlogCategories&cPath=' . $cPath . '&action=new_category'), 'info') . '&nbsp;';
    echo HTML::button($CLICSHOPPING_Blog->getDef('button_new_ticket'), null, $CLICSHOPPING_Blog->link('BlogContent&cPath=' . $cPath . '&action=new_product'), 'success') . ' ';
  }
  echo HTML::form('delete_all', $CLICSHOPPING_Blog->link('BlogContent&DeleteAll&cPath=' . $cPath));
?>
...
// Permettre l'affichage des groupes en mode B2B
            if (MODE_B2B_B2C == 'true') {
              ...
              $QcustomersGroup = $CLICSHOPPING_Blog->db->prepare('select customers_group_name
                                                             from :table_customers_groups
                                                             where customers_group_id = :customers_group_id
                                                           ');
              $QcustomersGroup->bindint(':customers_group_id', $QblogContent->valueInt('customers_group_id'));
              $QcustomersGroup->execute();

              $customers_group = $QcustomersGroup->fetch();

              if (is_null($QcustomersGroup->value('customers_group_name'))) {
...
// Get blog_categories_id for product if search
            if (isset($_POST['search'])) $cPath = $QblogContent->valueInt('blog_categories_id');
            ?>
            <td>
...
// Permettre l'affichage des groupes en mode B2B
            ...
            <td class="text-center">
              <?php
                if ($QblogContent->valueInt('blog_content_status') == 1) {
                  echo '<a href="' . $CLICSHOPPING_Blog->link('BlogContent&SetFlag&flag=0&pID=' . $QblogContent->valueInt('blog_content_id') . '&cPath=' . $cPath) . '"><i class="fas fa-check fa-lg" aria-hidden="true"></i></a>';
                } else {
                  echo '<a href="' . $CLICSHOPPING_Blog->link('BlogContent&SetFlag&flag=1&pID=' . $QblogContent->valueInt('blog_content_id') . '&cPath=' . $cPath) . '"><i class="fas fa-times fa-lg" aria-hidden="true"></i></a>';
                }
              ?>
            </td>
            ...
            <td class="text-end">
              <?php
                ...
                echo '<a href="' . $CLICSHOPPING_Blog->link('BlogContent&cPath=' . $cPath . '&pID=' . $QblogContent->valueInt('blog_content_id') . '&action=new_product') . '">' . HTML::image($CLICSHOPPING_Template->getImageDirectory() . 'icons/edit.gif', $CLICSHOPPING_Blog->getDef('image_edit')) . '</a>';
                echo '&nbsp;';
                echo '<a href="' . $CLICSHOPPING_Blog->link('BlogContent&cPath=' . $cPath . '&pID=' . $QblogContent->valueInt('blog_content_id') . '&action=move_product') . '">' . HTML::image($CLICSHOPPING_Template->getImageDirectory() . 'icons/move.gif', $CLICSHOPPING_Blog->getDef('image_move')) . '</a>';
                echo '&nbsp;';
                echo '<a href="' . $CLICSHOPPING_Blog->link('BlogContent&cPath=' . $cPath . '&pID=' . $QblogContent->valueInt('blog_content_id') . '&action=copy_to') . '">' . HTML::image($CLICSHOPPING_Template->getImageDirectory() . 'icons/copy.gif', $CLICSHOPPING_Blog->getDef('image_copy_to')) . '</a>';
                echo '&nbsp;';
                echo '<a href="' . $CLICSHOPPING_Blog->link('BlogContent&Unpack&cPath=' . $cPath . '&pID=' . $QblogContent->valueInt('blog_content_id') . '&action=unpack') . '">' . HTML::image($CLICSHOPPING_Template->getImageDirectory() . 'icons/unpack.gif', $CLICSHOPPING_Blog->getDef('image_unpack')) . '</a>';
                echo '&nbsp;';
                echo '<a href="' . $CLICSHOPPING_Blog->link('BlogContent&cPath=' . $cPath . '&pID=' . $QblogContent->valueInt('blog_content_id') . '&action=delete_product') . '">' . HTML::image($CLICSHOPPING_Template->getImageDirectory() . 'icons/delete.gif', $CLICSHOPPING_Blog->getDef('image_delete')) . '</a>';
                echo '&nbsp;';
              ?>
            </td>
            </tr>
