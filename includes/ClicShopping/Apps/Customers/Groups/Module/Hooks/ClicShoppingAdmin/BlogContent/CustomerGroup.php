<?php
  ...
  class CustomerGroup implements \ClicShopping\OM\Modules\HooksInterface
  {
    ...
    {
      if (isset($_GET['pID'])) {
        $QblogContent = $this->app->db->prepare('select customers_group_id
                                                 from :table_blog_content
                                                 where blog_content_id = :blog_content_id
                                                ');

        $QblogContent->bindInt('blog_content_id', $_GET['pID']);
        $QblogContent->execute();

        return $QblogContent->valueInt('customers_group_id');
      }
    }
